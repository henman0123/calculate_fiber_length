...
function calculateFiberLengthCustom1(x1, y1, z1, x2, y2, z2) {
  ...
  let xyLength = Math.abs(y1 - y2) * 0.733; //只根據 y1、y2 值差值計算橫向距離
  ...
  let totalLength = xyLength + zLength + 1.2; //補正值 1.6 米(補正值為 0.6 米 + 1 米為含 OPEN RACK 上線槽的空間 8/3:補正值改為1.2米 114/1/2: 補正值改為1.35米)
  console.log(totalLength);
  return Math.ceil(totalLength);
}
